...
export function brokeredPreviewStorage() {
  ...
  const request = (type: string, key: string, value?: string): Promise<{ ok: boolean; value?: string | null } | null> =>
    new Promise((resolve) => {
      ...
      // Declared here (not above) so it can be const: `finish` closes over it
      // and only dereferences it when invoked, which is always after this line
      // because message dispatch and the timeout are both asynchronous.
