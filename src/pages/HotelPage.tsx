...
// Hotel page mirrors gym booking flow; QR component reserved for hotel-specific use later
...
const HotelPage = () => {
  ...
  const handlePayment = useCallback(async () => {
    if (!selectedService || !selectedDate || !selectedTime || !hotel || !canProceed) return;
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-clip">
      {hotel && (
        <>
        <SEO
          title={t("hotels.detailSeoTitle", { hotel: hotel.name })}
          description={t("hotels.detailSeoDescription", {
            hotel: hotel.name,
            citySuffix: hotel.city_name ? ` ${i18n.language === "en" ? "in" : "in"} ${hotel.city_name}` : "",
          })}
          keywords={t("hotels.detailSeoKeywords", {
            hotel: hotel.name,
        ...
        <Helmet>
          <script type="application/ld+json">{JSON.stringify({
            ...
            url: `https://massavo.com/hotel/${hotel.id}`,
            image: hotel.image_url || undefined,
          })}</script>
        </Helmet>
        </>
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span>({hotel.review_count ?? 0} {t("gymPage.reviews")})</span>
                  </div>
                </div>
              </div>
              <div className="flex flex-col gap-2 text-sm">
                <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  {hotel.open_hours || "Sa-So: 10:00-18:00"}
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="lg:col-span-1">
                ...
                <div className="mt-6">
                  <HotelQRCode
                    hotelId={hotel.id}
                    hotelName={hotel.name}
                    cityName={hotel.city_name || undefined}
                    size={200}
                  />
                </div>
